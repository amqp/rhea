...
function write_uuid(buffer, value, offset) {
    var i = (buffer && offset) || 0, ii = 0;
    buffer = buffer || [];
    value.toLowerCase().replace(/[0-9a-f]{2}/g, function(oct) {
        if (ii < 16) {
            buffer[i + ii++] = HEX_TO_BYTE[oct];
        }
    });
    while (ii < 16) {
        buffer[i + ii++] = 0;
...
function read_uuid(buffer, offset) {
    var i = offset || 0;
    return BYTE_TO_HEX[buffer[i++]] + BYTE_TO_HEX[buffer[i++]] +
           BYTE_TO_HEX[buffer[i++]] + BYTE_TO_HEX[buffer[i++]] + '-' +
           BYTE_TO_HEX[buffer[i++]] + BYTE_TO_HEX[buffer[i++]] + '-' +
           BYTE_TO_HEX[buffer[i++]] + BYTE_TO_HEX[buffer[i++]] + '-' +
           BYTE_TO_HEX[buffer[i++]] + BYTE_TO_HEX[buffer[i++]] + '-' +
           BYTE_TO_HEX[buffer[i++]] + BYTE_TO_HEX[buffer[i++]] +
...
define_type('Uuid', 0x98, {'write':write_uuid, 'read':read_uuid});//TODO: convert to/from stringified form?
