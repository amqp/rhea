...
function validateBufferSize(bufferSize) {
    if (bufferSize && Number.isInteger(Number(bufferSize))) {
        return Number(bufferSize);
    }
    return DEFAULT_BUFFER_SIZE;
}

function getBufferSize(bufferSize, type) {
    if (!bufferSize) {
        return DEFAULT_BUFFER_SIZE;
    }
    ...
    return validateBufferSize(bufferSize[type]);
}

var Outgoing = function (connection, bufferSize) {
    this.deliveries = new CircularBuffer(getBufferSize(bufferSize, 'outgoing'));
    this.updated = [];
...
var Incoming = function (bufferSize) {
    this.deliveries = new CircularBuffer(getBufferSize(bufferSize, 'incoming'));
    this.updated = [];
    this.next_transfer_id = 0;
...
var Session = function (connection, local_channel, bufferSize) {
    this.connection = connection;
    this.bufferSize = bufferSize;
    this.outgoing = new Outgoing(connection, bufferSize);
    this.incoming = new Incoming(bufferSize);
...
Session.prototype._reconnect = function() {
    ...
    this.outgoing = new Outgoing(this.connection, this.bufferSize);
    this.incoming = new Incoming(this.bufferSize);
    this.remote = {'handles':{}};
    for (var l in this.links) {
