...
Incoming.prototype.on_transfer = function(frame, receiver) {
    ...
    if (receiver.is_open()) {
        ...
        if (last && last.incomplete) {
            ...
            current.accept = function () { this.update(true, message.accepted().described()); };
            current.release = function (params) {
                if (params) {
                    this.update(true, message.modified(params).described());
                } else {
                    this.update(true, message.released().described());
                }
            };
