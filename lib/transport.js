...
Transport.prototype.encode = function (frame) {
    ...
    log.frames('[' + this.identifier + '] SENT: ' + JSON.stringify(frame));
    this.pending.push(buffer);
    /*
    var buffer = new Buffer(1024);
    while (true) {
        try {
            var len = frames.write_frame(buffer, frame);
            log.frames('[' + this.identifier + '] SENT: ' + JSON.stringify(frame));
            this.pending.push(buffer.slice(0, len));
            return;
        } catch (e) {
            log.frames('caught ' + e + '; expanding buffer from ' + buffer.length);
