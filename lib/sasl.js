...
PlainServer.prototype.start = function(response) {
    ...
    if (fields.length !== 3) {
        throw Error('Unexpected response in PLAIN, got ' + fields.length + ' fields, expected 3');
    }
 ...
 */
var SaslServer = function (amqp_transport, mechanisms) {
    this.transport = new Transport(amqp_transport.identifier, SASL_PROTOCOL_ID, frames.TYPE_SASL, this);
    this.next = amqp_transport;
...
SaslServer.prototype.recreate = function () {
    return new SaslServer(this.next.recreate(), this.mechanisms);
};
...
var SaslClient = function (amqp_transport, mechanisms) {
    this.transport = new Transport(amqp_transport.identifier, SASL_PROTOCOL_ID, frames.TYPE_SASL, this);
    this.next = amqp_transport;
    this.mechanisms = mechanisms;
    this.mechanism = undefined;
...
SaslClient.prototype.recreate = function () {
    return new SaslClient(this.next.recreate(), this.mechanisms);
};
...
SaslClient.prototype.on_sasl_mechanisms = function (frame) {
    ...
    if (this.mechanism) {
        ...
        throw Error('No suitable mechanism; server supports ' + frame.performative.sasl_server_mechanisms);
    }
};
...
SaslClient.prototype.on_sasl_outcome = function (frame) {
    ...
    default:
        this.transport.write_complete = true;
        throw Error("Failed to authenticate: " + frame.performative.code);
    }
};
