...
frames.write_amqp_frame = function(buffer, channel, performative, payload) {
    return this.write_frame(buffer, {'channel': channel, 'type': frames.TYPE_AMQP, 'performative': performative, 'payload': payload});
};
frames.write_sasl_frame = function(buffer, performative) {
    return this.write_frame(buffer, {'channel': 0, 'type': frames.TYPE_SASL, 'performative': performative});
};
frames.amqp_frame = function(channel, performative, payload) {
    return {'channel': channel, 'type': frames.TYPE_AMQP, 'performative': performative, 'payload': payload};
};
