...
frames.read_header = function(buffer) {
    var offset = 4;
    var header = {};
    header.name = buffer.toString('ascii', 0, offset);
    if (header.name !== 'AMQP') {
        throw Error('Invalid protocol header for AMQP ' + header.name);
    }
...
frames.read_frame = function(buffer) {
    ...
    if (frame.type === TYPE_AMQP) {
        frame.channel = reader.read_uint(2);
    } else if (frame.type === TYPE_SASL) {
        reader.skip(2);
    } else {
...
frames.write_frame = function(buffer, frame) {
    var writer = new types.Writer(buffer);
    ...
    writer = new types.Writer(buffer);
    writer.write_uint(written, 4);//fill in the size
    return written;
};

frames.write_amqp_frame = function(buffer, channel, performative, payload) {
    return this.write_frame(buffer, {'channel': channel, 'type': TYPE_AMQP, 'performative': performative, 'payload': payload});
};
frames.write_sasl_frame = function(buffer, performative) {
    return this.write_frame(buffer, {'channel': 0, 'type': TYPE_SASL, 'performative': performative});
};
...
define_frame(TYPE_AMQP, open);
define_frame(TYPE_AMQP, begin);
define_frame(TYPE_AMQP, attach);
define_frame(TYPE_AMQP, flow);
define_frame(TYPE_AMQP, transfer);
define_frame(TYPE_AMQP, disposition);
define_frame(TYPE_AMQP, detach);
define_frame(TYPE_AMQP, end);
define_frame(TYPE_AMQP, close);
