...
function define_map_section(def) {
    ...
    var unwrap = function (msg, section) {
        msg[name] = types.unwrap_map(section);
    };
    var wrap = function (sections, msg) {
        if (msg[def.name]) {
...
/*
    if (obj.header) {
        sections.push(obj.header);
    }
    if (obj.delivery_annotations) {
        sections.push(types.described(types.wrap_ulong(0x71), types.wrap_map(obj.delivery_annotations)));
    }
    if (obj.message_annotations) {
        sections.push(types.described(types.wrap_ulong(0x72), types.wrap_map(obj.message_annotations)));
    }
    if (obj.properties) {
        sections.push(obj.properties);
    }
    if (obj.application_properties) {
        sections.push(types.described(types.wrap_ulong(0x74), types.wrap_map(obj.application_properties)));
    }
    if (obj.body) {
        //TODO: support data and sequence sections
        sections.push(types.described(types.wrap_ulong(0x77), types.wrap(obj.body)));
    }
    if (obj.footer) {
        sections.push(types.described(types.wrap_ulong(0x78), types.wrap_map(obj.footer)));
    }
*/
    var buffer = new Buffer(2048);//TODO: proper buffer sizing
    ...
    var data = buffer.slice(0, writer.position);
    log.message('encoded ' + writer.position + ' bytes');
    return data;
}
