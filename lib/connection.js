...
Connection.prototype.reset = function() {
    ...
    //reset transport
    this.amqp_transport = this.amqp_transport.recreate();
    this.sasl_transport = undefined;
    this.transport = this.amqp_transport;
...
Connection.prototype._connect = function (details) {
    var connector = get_connect_fn(details);
    this.init(connector(details.port, details.host, details.options, this.connected.bind(this)));
    return this;
};
...
Connection.prototype.init = function (socket) {
    ...
    if (this.is_server) {
        ...
        if (this.socket.encrypted && this.socket.authorized && this.get_option('enable_sasl_external', false)) {
            mechs = sasl.server_add_external(mechs ? utils.clone(mechs) : {});
        }
        if (mechs) {
            this.sasl_transport = new sasl.Server(this.amqp_transport, mechs);
        }
    } else {
...
Connection.prototype.output = function () {
    if (this.socket) {
        ...
        if (((this.is_closed() && this.state.has_settled()) || this.abort_idle) && !this.transport.has_writes_pending()) {
            this.socket.end();
...
Connection.prototype._disconnected = function () {
    if (!this.is_closed()) {
        ...
        this.dispatch('disconnected', this._context());
        if (!this.is_server && this.options.reconnect) {
