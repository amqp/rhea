...
function session_per_connection(conn, bufferSize) {
    var ssn = null;
    return {
        'get_session' : function () {
            if (!ssn) {
                ssn = conn.create_session(bufferSize);
...
var Connection = function (options, container) {
    ...
    this.session_policy = session_per_connection(this, this.bufferSize);
...
Connection.prototype.create_session = function (bufferSize) {
    var i = 0;
    while (this.local_channel_map[i]) i++;
    var session = new Session(this, i, bufferSize);
...
Connection.prototype.on_begin = function (frame) {
    ...
    if (frame.performative.remote_channel === null || frame.performative.remote_channel === undefined) {
        //peer initiated
        session = this.create_session(this.bufferSize);
