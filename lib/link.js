...
Sender.prototype.send = function (msg, tag, format) {
    ...
    if(this.get_option('tracing', true)){
        ...
        // span.SPAN_KIND_MESSAGING_PRODUCER;
        //Tags.SPAN_KIND_MESSAGING_PRODUCER

        span.setTag('inserted_by', 'proton-message-tracing');
        span.setTag(opentracing.Tags.PEER_HOSTNAME, "localhost");
        span.setTag(opentracing.Tags.SPAN_KIND, "producer");
        span.setTag("peer.address", "amqp://localhost:5672/examples");
        span.setTag("message_bus.destination", "examples");
        temp++;
        span.setTag('delivery-tag', `${temp}`);
        ...
        if(msg.message_annotations == undefined){
            msg.message_annotations = { 'x-opt-qpid-tracestate': headers };
            //   msg.annotations = "test"
        ...
        //   let bufferHeaders = new Buffer(data);

        // span.finish();
    }
    var payload = format === undefined ? message.encode(msg) : msg;
    let bufferHeaders = new Buffer(payload);
    // console.log(bufferHeaders.toString())
    //   console.log(payload)
    let delivery = this.session.send(this, tag ? tag : this.next_tag(), payload, format);
    if(this.get_option('tracing', true)){
        delivery.span = span;
        this.observers.on('settled', (e)=>{
            span.finish();
        });
    }
...
var Receiver = function (session, name, local_handle, opts) {
    ...
    this.observers.on('message',(e)=>{
        ...
        if(message.message_annotations == undefined){
            let ctx = this.tracer.startSpan('amqp-deliver-receive', tags = span_tags);
            ctx.finish();
        } else {
            let headers = message.message_annotations[_trace_key];
            let bufferHeaders = new Buffer(headers['uber-trace-id']);
            let traceId = { 'uber-trace-id': bufferHeaders.toString()};
            this.tracer = opentracing.globalTracer();
            let span_ctx = this.tracer.extract(opentracing.FORMAT_TEXT_MAP, traceId);
